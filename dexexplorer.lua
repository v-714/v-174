...
local function main()
	...
	-- Data table that holds filter info, including enable flags and function references
local filterData = {
    isa = {
        enabled = true,
...
-- Build the Comparison table dynamically only for enabled filters
for filterName, filterInfo in pairs(filterData) do
    if filterInfo.enabled and type(filterInfo.func) == "function" then
        Explorer.SearchFilters.Comparison[filterName] = filterInfo.func
    end
end
...
local function main()
    ...
    -- Table of subproperties keyed by typeName
    local typeSubProps = {
        Vector2 = {
            {".X", {Name = "float"}},
            {".Y", {Name = "float"}},
        ...
        CFrame = {
            {".Position", {Name = "Vector3"}},
            {".RightVector", {Name = "Vector3"}},
            {".UpVector", {Name = "Vector3"}},
            {".LookVector", {Name = "Vector3"}},
        },
        UDim = {
            {".Scale", {Name = "float"}},
            {".Offset", {Name = "int"}},
        ...
        PhysicalProperties = {
            {".Density", {Name = "float"}},
            {".Elasticity", {Name = "float"}},
            {".ElasticityWeight", {Name = "float"}},
            {".Friction", {Name = "float"}},
            {".FrictionWeight", {Name = "float"}},
        },
        Ray = {
            {".Origin", {Name = "Vector3"}},
            {".Direction", {Name = "Vector3"}},
        },
        NumberRange = {
            {".Min", {Name = "float"}},
            {".Max", {Name = "float"}},
    ...
    local subProps = typeSubProps[typeName]
    if subProps then
        for i, v in ipairs(subProps) do
            -- v can have 2 or 3 elements: path, meta, optional custom name
            if v[3] then
                result[i] = makeSubProp(prop, v[1], v[2], v[3])
            else
                result[i] = makeSubProp(prop, v[1], v[2])
            end
        end
    end

    -- Special case for SoundId property in Sound class
    ...
    if not editor then
        ...
        editor.OnMoreColors:Connect(function()
            -- Special Case: If editing BasePart.BrickColor, switch to BasePart.Color color editor
            if editor.CurrentProp and editor.CurrentProp.Class == "BasePart" and editor.CurrentProp.Name == "BrickColor" then
                ...
                -- Optional: handle other cases or just close editor without opening Color editor
                editor:Close()
            end
        end)
...
local function main()
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if not success or not source then 
			source, PreviousScr = "-- Source failed to decompile", nil 
		else 
			PreviousScr = scr 
			source = ("-- Decompiled in %.3f seconds\n\n%s"):format(duration, source)
		end
	...
	ScriptViewer.Init = function()
		...
		codeFrame.Frame.Position = UDim2.new(0, 0, 0, 30)
		codeFrame.Frame.Size = UDim2.new(1, 0, 1, -30)
		...
		copy.Size = UDim2.new(buttonWidth, 0, 0, buttonHeight)
		copy.Text = "Copy to Clipboard"
		copy.TextColor3 = Color3.new(1, 1, 1)
		copy.Parent = window.GuiElems.Content
		...
		save.Position = UDim2.new(buttonWidth, 0, 0, 0)
		save.Size = UDim2.new(buttonWidth, 0, 0, buttonHeight)
		...
		dumpbtn.Position = UDim2.new(buttonWidth*2, 0, 0, 0)
		dumpbtn.Size = UDim2.new(buttonWidth, 0, 0, buttonHeight)
		dumpbtn.Text = "Dump Functions"
		dumpbtn.TextColor3 = Color3.new(1, 1, 1)
		dumpbtn.Parent = window.GuiElems.Content
		...
		dumpbtn.MouseButton1Click:Connect(function()
			...
			pcall(function()
				...
				local function tabDump(tbl, indent)
					indent = indent or 1
					if seen[tbl] then
						dump = dump .. string.rep("    ", indent) .. "(recursive table)\n"
						return
					end
					seen[tbl] = true
					for k, v in pairs(tbl) do
						if type(v) == "table" then
							dump = dump .. string.rep("    ", indent) .. tostring(k) .. " = {\n"
							tabDump(v, indent + 1)
							dump = dump .. string.rep("    ", indent) .. "}\n"
						else
							dump = dump .. string.rep("    ", indent) .. tostring(k) .. " = " .. tostring(v) .. "\n"
						end
					end
				end
				...
				local function dumpFunction(func)
					...
					dump = dump .. ("Function: %s | Source: %s\n"):format(info.name or "(anonymous)", info.short_src)
					dump = dump .. "  Upvalues:\n"
					for i, v in ipairs(getupvalues(func) or {}) do
						if type(v) == "table" then
							dump = dump .. ("    [%d]: table {\n"):format(i)
							tabDump(v, 2)
							dump = dump .. "    }\n"
					...
					dump = dump .. "  Constants:\n"
					for i, v in ipairs(getconstants(func) or {}) do
						dump = dump .. ("    [%d]: %s\n"):format(i, tostring(v))
					end
					dump = dump .. ("  %s\n"):format(("="):rep(50))
				end
				...
				for _, f in pairs(getgc()) do
					if typeof(f) == "function" and getfenv(f).script == PreviousScr then
						dumpFunction(f)
					end
				end
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="V1.1",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
